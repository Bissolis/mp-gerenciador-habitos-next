...
const daysInMonth = getDaysInMonth(10, currentYear)
        ...
        <section className="w-full my-2 rounded-md bg-neutral-800">
                <div className="flex justify-between mx-2 my-4 font-sans text-neutral-400">
                    <button onClick={goToPreviousMonth}>
                        <ArrowIcon width={12} height={12} className="text-neutral-400"/>
                    </button>
                    <span>
                        {getFullDateString()}
                    </span>
                    <button onClick={goToNextMonth}>
                        <ArrowIcon width={12} height={12} className="rotate-180 text-neutral-400"/>
                    </button>
                </div>
                ...
                <div className="grid w-full grid-cols-7 mt-2">
                    ...
                    {daysInMonth.map((day, index) => (
                        <div key={index} className="flex flex-col items-center p-2" onClick={() => { if (!day) return; toggleHabit({ habit, habitStreak, date: getDayString(day), done: habitStreak?.[getDayString(day)] }); }}>
